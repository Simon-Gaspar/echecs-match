...
async function fetchMonthPage(month: string): Promise<string> {
    const url = `${BASE_URL}?country=BEL&month=${month}`;
...
export async function fetchBelgianTournaments(): Promise<Tournament[]> {
    console.log("Scraping Belgian tournaments (chess-calendar.eu)...");
    ...
    try {
        ...
        for (const month of months) {
            console.log(`  Fetching Belgium month ${month}...`);
            ...
            $('div.calItem').each((_, el) => {
                ...
                const lower = name.toLowerCase();
                if (lower.includes('scolaire') || lower.includes('school') || lower.includes('scholier')) return;
                if (lower.includes('équipe') || lower.includes('team') || lower.includes('ploeg')) return;

                const dedupeKey = `${name.toLowerCase()}-${startDate}`;
                if (seenNames.has(dedupeKey)) return;
                seenNames.add(dedupeKey);

                let city = $el.find('div.city').text().trim().replace(/^,\s*/, '').replace(/,\s*BEL\s*$/, '').trim();
                if (!city || city === ',') city = 'Belgique';

                let lat = 50.8503; // Default: Brussels
                let lng = 4.3517;
                ...
                const id = `belgium-${Buffer.from(idPayload).toString('base64').replace(/[/+=]/g, '').slice(-16)}`;
        ...
        console.log(`  Total Belgian tournaments: ${tournaments.length}`);
        return tournaments;

    } catch (e) {
        console.error("Belgian scraping failed:", e);
        return [];
    }
}
