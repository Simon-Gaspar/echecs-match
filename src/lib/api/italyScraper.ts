...
function detectFormat(name: string): 'Blitz' | 'Rapide' | 'Lent' {
    ...
    if (
        ...
        lower.includes('semi-rapido') ||
        lower.includes('attivo') ||
...
async function fetchMonthPage(month: string): Promise<string> {
    const url = `${BASE_URL}?country=ITA&month=${month}`;
...
export async function fetchItalianTournaments(): Promise<Tournament[]> {
    console.log("Scraping Italian tournaments (chess-calendar.eu)...");
    ...
    try {
        ...
        for (const month of months) {
            console.log(`  Fetching Italy month ${month}...`);
            ...
            $('div.calItem').each((_, el) => {
                ...
                const lower = name.toLowerCase();
                if (lower.includes('scuola') || lower.includes('school')) return;
                if (lower.includes('squadre') || lower.includes('team') || lower.includes('coppa')) return;

                // Deduplicate by name + date (same tournament appears from multiple sources)
                const dedupeKey = `${name.toLowerCase()}-${startDate}`;
                if (seenNames.has(dedupeKey)) return;
                seenNames.add(dedupeKey);

                // Parse city
                let city = $el.find('div.city').text().trim().replace(/^,\s*/, '').replace(/,\s*ITA\s*$/, '').trim();
                if (!city || city === ',') city = 'Italia';
                ...
                const id = `italy-${Buffer.from(idPayload).toString('base64').replace(/[/+=]/g, '').slice(-16)}`;
        ...
        console.log(`  Total Italian tournaments: ${tournaments.length}`);
        return tournaments;

    } catch (e) {
        console.error("Italian scraping failed:", e);
        return [];
    }
}
