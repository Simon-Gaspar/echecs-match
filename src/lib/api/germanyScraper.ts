...
function detectFormat(name: string): 'Blitz' | 'Rapide' | 'Lent' {
    ...
    if (lower.includes('blitz') || lower.includes('bullet') || lower.includes('lightning') || lower.includes('schnell') || lower.includes('5 min') || lower.includes("5'") || lower.includes('3+2')) return 'Blitz';
    if (lower.includes('rapid') || lower.includes('schnellschach') || lower.includes('aktiv') || lower.includes('15 min') || lower.includes('15+') || lower.includes('20 min') || lower.includes('25 min') || lower.includes('30 min')) return 'Rapide';
    return 'Lent';
}

async function fetchMonthPage(month: string): Promise<string> {
    const url = `${BASE_URL}?country=GER&month=${month}`;
    const res = await fetch(url, { headers: { 'User-Agent': 'EchecsMatchApp/1.0 (chess tournament aggregator)' } });
    return await res.text();
}

export async function fetchGermanTournaments(): Promise<Tournament[]> {
    console.log("Scraping German tournaments (chess-calendar.eu)...");
    try {
        const now = new Date();
        const months: string[] = [];
        for (let i = 0; i < 3; i++) {
        ...
        for (const month of months) {
            console.log(`  Fetching Germany month ${month}...`);
            ...
            $('div.calItem').each((_, el) => {
                ...
                const lower = name.toLowerCase();
                if (lower.includes('schulschach') || lower.includes('school') || lower.includes('jugend')) return;
                if (lower.includes('mannschaft') || lower.includes('team')) return;

                const dedupeKey = `${name.toLowerCase()}-${startDate}`;
                if (seenNames.has(dedupeKey)) return;
                seenNames.add(dedupeKey);

                let city = $el.find('div.city').text().trim().replace(/^,\s*/, '').replace(/,\s*GER\s*$/, '').trim();
                if (!city || city === ',') city = 'Deutschland';

                let lat = 52.5200; // Default: Berlin
                let lng = 13.4050;
                ...
                const id = `germany-${Buffer.from(idPayload).toString('base64').replace(/[/+=]/g, '').slice(-16)}`;
        ...
        console.log(`  Total German tournaments: ${tournaments.length}`);
        return tournaments;
    } catch (e) {
        console.error("German scraping failed:", e);
        return [];
    }
}
